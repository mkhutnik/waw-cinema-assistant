...
class Movie:
    ia = imdb.IMDb()
    def __init__(self, time, base_url, title=None, cinema=None, year=None,
                 rating=None):
    ...
    def set_title(self):
        self.title = CinemaScraper(self.base_url).find_elements_by_tag('h1')[
            0].text
        return self.title
    ...
    def set_year(self):
        try:
            year_str = str([i.find_all_next('li') for i in
                            CinemaScraper(self.base_url)._html.find_all('ul',
                                                                        class_='movie-info')][
                               0][1])
